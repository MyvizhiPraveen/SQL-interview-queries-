...
with cte1 as
(select *,row_number() over(partition by position_id) as rn from job_employees), -- task1
cte2 as
(
Select * from(
With recursive job_pos as(
Select id, title, groupsid, levels, payscale, 1 as totalpost from job_positions
UNION ALL
Select cte.id, cte.title, cte.groupsid, cte.levels, cte.payscale, (cte.totalpost + 1) as totalpost
from job_pos as cte
JOIN job_positions p ON p.id = cte.id and p.totalpost > cte.totalpost)
Select * from job_pos order by title) as x)

select p.title as TITLE,p.groupsid as GROUPSID,p.levels as LEVEL,p.payscale as PAYSCALE,ifnull(c.name,'Vacant') as EMPLOYEE_NAME
from cte2 p
left join
cte1 c
on p.id = c.position_id and p.totalpost = c.rn
order by p.groupsid;
...
-- Q2.Display highest and lowest salary from the given employee table based on each department and display the employee record
...
select *,max(salary) over(partition by dept order by salary desc) as highest_salary,
       min(salary) over(partition by dept order by salary desc range between unbounded preceding and unbounded following)
       as lowest_salary 
       from employee;
-- Frame clause range between unbounded preceding and unbounded following ensure that min function has access to all rows.
...
-- to fetch data from the previous record, use lag windowfunction
select *,cumulative_distance-lag(cumulative_distance,1,0)
...
insert into source_dest values('Bangalore','Hyderabad', 400);
insert into source_dest values('Hyderabad','Bangalore',400);
insert into source_dest values('Mumbai','Delhi',400);
insert into source_dest values('Delhi','Mumbai',400);
insert into source_dest values('Chennai','Pune',400);
insert into source_dest values('Pune','Chennai',400);
select * from source_dest;
-- creating a row_number window function , to create a unique identifier column
with cte as (
...
-- Q5 Recursive SQL problem  (same recursive query executed several times until a condition is met)
-- Q5a.Display numbers 1 to 10 without using any build in function
...
-- Q5b.Find the hierarchy of employees under a given manager "Asha"
drop table if exists rec_emp_details;
create table rec_emp_details
( id int primary key,
  name varchar(100),
  manager_id int,
  salary int,
...
insert into rec_emp_details (id,name,manager_id,salary,designation) values
							(1,'Shripath',null, 10000,'CEO'),
                            ...
                            (7,'Asha',1,4200,'CTO'),
                            (8,'Mariyam',1,3800,'Manager'),
                            (9,'Reshma',8,2000,'Business Analyst'),
                            (10,'Akshay',8,2500,'Java Developer')
                            ;
select * from rec_emp_details;
with recursive emp_hierarchy as
	(select id,name,manager_id,designation,1 as lvl
    from rec_emp_details where name ="Asha"  -- refer schema name before table name --MS SQL-remove recursive keyword use union all
    union
    select e.id,e.name,e.manager_id,e.designation,lvl+1 
    from emp_hierarchy H
    join rec_emp_details e
    on H.id = e.manager_id
    )
select * from emp_hierarchy;

-- Q5c.Find the hierarchy of managers for a given employee 'David'
with recursive man_hierarchy as
	(select id,name,manager_id,designation,1 as lvl
    from rec_emp_details where name ="David" -- refer schema name before table name --MS SQL-remove recursive keyword use union all
    union
    select e.id,e.name,e.manager_id,e.designation,lvl+1 
    from man_hierarchy H
    join rec_emp_details e
    on H.manager_id =e.id
    )
select H2.id as emp_id,H2.name as emp_name,E2.name as manager_name,H2.lvl as level
from man_hierarchy H2
join 
rec_emp_details E2 on E2.id =H2.manager_id;

-- Q5d. Ungroup the given inout data
...
with recursive ungroup_items as
	(select id,item_name,total_count, 1 as level -- base query /1st output
    from travel_items
    union
    select u.id,u.item_name,u.total_count-1,level +1 as level
    from ungroup_items u
    join 
    travel_items t on u.id = t.id and u.item_name =t.item_name 
    where u.total_count > 1  -- recursive query until a condition is met
    )
select id,item_name,level from ungroup_items;
-- Q6 : IPL Matches ---
-- Q6a.write a query such that each team played only once with all other teams
...
select * from teams;
with iplteam as 
    (select *,row_number() over(order by team_name) as id from teams)
select ip.team_name as Team,op.team_name as Opponent
from iplteam ip
join
iplteam op on ip.id < op.id 
order by Team;

-- Q6b.Q6a.write a query such that each team played with evry other team twice
select * from teams;
with iplteam as
     ...
     iplteam op on ip.id <> op.id 
     order by Team;
-- Q7 Find difference in average sales --- 
...
commit;
with cte as
     (select year_id, month_id, avg(sales) as avg_sales_per_month,to_char(order_date,"MON") as mon
     from Sales_order s
